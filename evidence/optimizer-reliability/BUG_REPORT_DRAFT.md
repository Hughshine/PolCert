...
`phase-dump-pluto-fork` fork used during PolCert development; official Pluto did not
...
| `phase-dump-pluto-fork` | `historical-bug-witness-pluto-snapshot` (`master`, 2026-05-05) | Source re-audit on 2026-08-28 | The five unresolved code paths are present. This fork also contains the conditional diamond-restore regression described below. |
| `phase-dump-pluto-fork` | `diamond-fix-snapshot` (`diamond-fix-branch`) | Built and executed on 2026-08-28 | The diamond regression is fixed on this branch. The other five witnesses remain active PolCert regression tests. |
| `phase-dump-pluto-fork` | `ordinary-fixed-pluto-snapshot` (`fixed-regression-branch`) | Built and executed on 2026-08-29; upstream `make check` passed | All six minimized probes now produce the source result or reject the illegal candidate before code generation. This is the ordinary artifact baseline. |
...
The phase-dump patch in `phase-dump-pluto-fork` commit `diamond-regression-introduction-snapshot` made that call
conditional on `options->intratileopt`; the regression remained present at the
audited predecessor `diamond-regression-snapshot`. The call restores a hyperplane temporarily
...
Fork commit `diamond-fix-snapshot` on
`diamond-fix-branch` restores the unconditional call
and adds an executable regression. The fixed branch and official latest both
print `20`; `historical-phase-dump-branch` does not yet contain this fix.
...
The target selects `phase-dump-pluto-fork@historical-bug-witness-pluto-snapshot` explicitly and checks more than
...
5. parallel annotations require a fresh check of the
