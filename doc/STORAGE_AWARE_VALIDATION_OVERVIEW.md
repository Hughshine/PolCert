...
view and the `State.eq` output view.
...
as `public_semantic_refinement`.  The generic copy protocol itself is now also
packaged as
...
`public_semantic_refinement`.  `CInstrScalarExpansionWitness.v` is the first concrete
