...
cell that has already been filled by copy-in or local write.  `CopyProtocolValidator.v` packages both the bookkeeping-only and
...
entry with matching logical/physical cells and equal boundary values.
...
`reuse_lookup` is within the declared physical bounds.  The strongest
