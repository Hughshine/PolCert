...
`InstanceTraceWitness.v`, `CopyInstanceWitness.v`,
...
`checked_parameterized_family_pair_certificate_refines`, are the internal
soundness hooks for accepted storage certificates.  Concrete storage-family
files wrap them behind direct `public_semantic_refinement` endpoints: input
states match under the certificate's input view, the target executes, and the
source has a matching execution whose final state satisfies the certificate's
output view.  The older
...
including the pair-certificate soundness hooks, so downstream files do not
need to reach into `StateView.v` directly for the proof carrier.
...
loaded-before-use.  The accepted certificate also now projects the structured
bounded storage facts: privatization recovers the bounded value/core,
...
direct `public_semantic_refinement` over the certificate input and output
views.  The expanded
...
`InstanceTraceWitness.v` factors out the reusable role-trace layer: any event
trace with a finite event-to-role function can be checked positionally against
projected target instances, and the checker directly recovers length,
target-to-event, and event-to-target role facts.  `CopyInstanceWitness.v`
instantiates that generic layer for copy protocol events: copy-in/local events
must be internal target instances, while copy-out events must be commit-role
target instances.  The checked copy trace now directly projects both
target-to-event and event-to-target role facts and can be reinterpreted as the
generic instance-trace obligations.
...
are duplicate-free.  The checker now also exposes the ordered lookup/fold
witness directly: successful checking yields the concrete ordered value list
obtained from the merge order, and if that ordered lookup is supplied by a
later trace proof, the checked fold result can be recovered without unpacking
the whole value-obligation record.  The algebra-law checker also projects closed,
...
endpoints whose conclusions are direct `public_semantic_refinement` statements
over certificate input/output views.  The generic pair-certificate machinery in
`StateView.v` remains the proof layer, not the feature-facing theorem surface.
