...
- Executed native compatibility mode:
  ...
  - default GLPK-enabled Pluto baseline result: `100 / 100` checks passed
...
| `--unrolljam` | Supported narrow | Pluto performs loop-body rewriting after codegen. PolOpt implements checked Loop IR post passes: complete unrolling for constant bounds, block/remainder unroll for variable-bound sequential loops, same-bound sibling-loop jam fusion through a per-candidate extracted validator, and verified cleanup. | Yes for the checked block/remainder route and for true jam shape when the local candidate validates; otherwise PolOpt falls back only for that candidate. The direct Pluto effect corpus currently covers every native clean effect it finds in the default set. | Keep expanding the corpus; add a Pluto-exact effect-policy gate only if compatibility should forbid extra checked post-pass effects. |
...
3. Treat semantic extensions as separate projects.
   - Pluto-exact unroll-jam effect policy, if the compatibility contract should
     suppress extra checked PolOpt post-pass effects when native Pluto codegen
     would not visibly change a fixture
...
```text
[pluto-compat-suite] OK (100 checks)
```
...
specific: exact unroll-jam effect-policy parity is separate from correctness
because the checked post-pass can legally optimize cases where native Pluto
codegen has no visible effect, first-class positive-stride Loop IR would make
stride syntax explicit but requires updating every downstream Loop pass/proof,
and full scalar privatization requires a checked scalar-private storage rewrite
