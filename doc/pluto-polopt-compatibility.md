...
- `--unrolljam` probe:
  ...
  - variable-bound cases now accept through `LoopUnroll.block_unroll_correct`, which emits a checked block loop with repeated factor offsets plus a residual remainder loop
...
| `--unrolljam` | Runs checked Loop IR post passes. Constant-bounded loops are fully unrolled. Variable-bound sequential loops use `--ufactor` as a checked block factor, emit repeated factor offsets, and leave a residual remainder loop. | native compat `const-unrolljam-constant-loop`, `const-unrolljam-ufactor-constant-loop`, `block-unrolljam-ufactor-variable-loop`, and `mixed-const-and-block-unrolljam`; end-to-end C `const_unroll`; `LoopUnroll.const_unroll_correct`; `LoopUnroll.block_unroll_correct` |
