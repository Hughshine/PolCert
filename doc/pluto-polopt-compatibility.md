...
- `--ufactor` probe:
  ...
  - without `--determine-tile-size`, the same flag is accepted with checked `--unrolljam`; constant-bound loops are fully unrolled and variable-bound sequential loops use the verified guarded peel-unroll route
- `--unrolljam` probe:
  ...
  - variable-bound cases such as `matmul.loop` now accept through `LoopUnroll.peel_unroll_correct`, which peels the requested factor under guards and leaves a residual loop
  - full Pluto-style block/remainder unroll-jam is still a gap because Pluto repeats the loop body inside a block loop and emits remainder handling after code generation
...
| `--unrolljam` | Runs checked Loop IR post passes. Constant-bounded loops are fully unrolled. Variable-bound sequential loops use `--ufactor` as a guarded peel count and leave a residual loop. This is a real semantic extension but still narrower than Pluto's full block/remainder unroll-jam. | native compat `const-unrolljam-constant-loop`, `const-unrolljam-ufactor-constant-loop`, `peel-unrolljam-ufactor-variable-loop`, and `mixed-const-and-peel-unrolljam`; end-to-end C `const_unroll`; `LoopUnroll.const_unroll_correct`; `LoopUnroll.peel_unroll_correct` |
...
| `--unrolljam` | Supported narrow | Pluto performs loop-body rewriting after codegen. PolOpt implements checked Loop IR post passes: complete unrolling for constant bounds, and guarded factor peel-unroll for variable-bound sequential loops. | Partly. | Extend `LoopUnroll` from guarded peel-unroll to full Pluto-style block/remainder unroll-jam with repeated-body block loops and explicit remainder generation. |
...
| `--ufactor` without `--determine-tile-size` | Supported with checked `--unrolljam` | With automatic tile-size selection absent, Pluto's general meaning is the unroll-jam factor. PolOpt records it as the guarded peel count for `LoopUnroll.peel_unroll_correct`; it is not passed to Pluto's scheduler oracle in this mode. | Partly. | Use the same factor in a future full block/remainder theorem if matching Pluto's exact unroll-jam output becomes necessary. |
...
| `--ufactor` | Supported with `--determine-tile-size` on tiled routes; supported as the checked `--unrolljam` peel factor otherwise | Pluto's automatic tile-size model rounds candidate sizes to a multiple of `ufactor`, even when `--nounrolljam` disables the post-codegen unroll-jam transform. Native compatibility mode passes `--ufactor` to Pluto only in this tile-size-model context. With `--unrolljam` and no `--determine-tile-size`, it is not passed to the scheduler oracle and is used by the verified LoopUnroll post pass. | Already supported for checked tile-size routes and checked unroll routes. | Add a block/remainder theorem if exact Pluto unroll-jam shape is needed. |
...
3. Treat semantic extensions as separate projects.
   - full Pluto-style block/remainder unroll-jam beyond the current guarded
     peel-unroll theorem
...
The current `polopt` surface already covers the core checked subset: affine scheduling, ordinary tiling, Pluto `tile.sizes` tile-size control via implicit and explicit `--tile-sizes-file` input, automatic tile-size-model controls including `--cache-size`, `--data-element-size`, and `--ufactor` under `--determine-tile-size`, Pluto `.fst` fusion-structure control via implicit and explicit `--fusion-structure` input, Pluto `.precut` partial-transformation control via implicit and explicit `--precut-file` input, identity tiling with checked ISS, second-level, ISS+second-level, and Pluto-hinted parallel/multipar composition, second-level tiling, ISS, one-loop parallelization, checked vector annotation for Pluto `--prevector`, checked unroll support for Pluto `--unrolljam` including constant-bound full unrolling and variable-bound guarded factor peel-unroll, `--multipar` parallelization up to two certified dimensions, sequential diamond tiling, full-diamond mode, conditional Candl dependence testing, conservative `--candldep --scalpriv` pass-through, and LP/DFP-family pass-through on the pinned GLPK-enabled Pluto baseline.

The remaining unsatisfied optimizer-facing semantic gaps are now narrow and
specific: exact Pluto-style block/remainder `--unrolljam` requires extending
the current peel-unroll theorem to repeated-body block loops plus remainder
generation, and full scalar privatization requires a checked scalar-private
