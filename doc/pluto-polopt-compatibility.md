...
- `--unrolljam` probe:
  ...
  - the same corpus records 5 fixtures where native Pluto shows no clean C change while PolOpt still applies an additional checked block/jam effect; this is a policy-alignment fact, not a correctness gap
  - `LoopJamLower` now exposes `checked_unrolljam_loop_with_plan`: an untrusted `list unrolljam_candidate` selects candidate depths, and the extracted checked pass still validates every selected block/jam transformation
  - `POLCERT_UNROLLJAM_POLICY=none` is a regression hook proving the selector can suppress all unroll-jam effects without rejecting the pipeline; the normal driver still uses the checked-all-depths selector
...
- Executed native compatibility mode:
  ...
  - default GLPK-enabled Pluto baseline result: `101 / 101` checks passed
...
| `--unrolljam` | Supported narrow | Pluto performs loop-body rewriting after codegen. PolOpt implements checked Loop IR post passes: complete unrolling for constant bounds, block/remainder unroll for variable-bound sequential loops, same-bound sibling-loop jam fusion through a per-candidate extracted validator, and verified cleanup. The heuristic/policy layer is now factored as an untrusted candidate-depth plan. | Yes for the checked block/remainder route and for true jam shape when the local candidate validates; otherwise PolOpt falls back only for that candidate. The direct Pluto effect corpus currently covers every native clean effect it finds in the default set, and the empty-selector regression shows the policy can suppress effects independently of correctness. | Replace the current checked-all-depths selector with a Pluto-profitability-compatible selector if exact effect policy parity is required. |
...
3. Treat semantic extensions as separate projects.
   - Pluto-exact unroll-jam effect policy: the plan hook exists, but the
     default selector is still checked-all-depths rather than Pluto's
     temporal-reuse/register-cost profitability model
...
specific: exact unroll-jam effect-policy parity is now a selector problem, not a
semantic preservation problem, because the checked post-pass can legally
optimize cases where native Pluto codegen has no visible effect. The remaining
work is to replace the current checked-all-depths selector with Pluto's
temporal-reuse/register-cost profitability model if exact parity is desired.
First-class positive-stride Loop IR would make stride syntax explicit but
requires updating every downstream Loop pass/proof, and full scalar
privatization requires a checked scalar-private storage rewrite before accepting
schedules that depend on privatized scalar instances. Frontend and backend
flags should remain outside the optimizer compatibility surface.
