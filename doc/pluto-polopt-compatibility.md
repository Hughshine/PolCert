...
- `--unrolljam` probe:
  ...
  - the checked side rejects the measurement if it observes any tiling-validation route, so the 6/6 result measures only the verified unroll-jam postpass and cannot be satisfied by a tiling path
  ...
  - per-candidate same-bound sibling-loop jam is now implemented: the extracted lowerer validates each fusion candidate locally at its Loop depth, using an abstract affine bound for the local proof when the real block bound contains `/`, and otherwise retains the checked order-preserving block/remainder output for that candidate
...
| `--multipar` | Supported on checked `--parallel` routes for every certified current dimension in the candidate list | The driver passes the flag to Pluto, parses OpenScop parallel loop hints, builds a route-specific candidate list from Pluto hints and current-space scattering depth, and calls the extracted verified `RawParallelCurrentMany*` compiler config. The Coq route filters certifiable dimensions before checked multi-cert parallel codegen, so this means "all accepted dimensions" rather than "all requested dimensions". | Supported for ordinary, second-level, identity-tiled, diamond, and ISS-aware parallel routes through `compile_verified_correct` plus the `Opt_parallel_current_many*` route theorems. The regression suite includes a three-dimensional independent loop that emits `parallel for i0`, `parallel for i1`, and `parallel for i2`. | Broaden fixtures; strict-mode positive behavior is covered by hint-only multipar checks, and an all-hints-rejected strict fixture would further tighten the regression suite. |
