...
- Executed native compatibility mode:
  ...
  - default GLPK-enabled Pluto baseline result: `70 / 70` checks passed
...
These are mostly not "missing Pluto optimization". They are backend features. A direct Pluto probe on `matmul.loop` shows that `--prevector` adds an OpenScop `<loop>` extension with directive `4`; `polopt` does not currently import that extension into a checked vector annotation. The same probe did not change the dumped schedule for `--unrolljam`, but the Pluto source applies unroll-jam to the CLAST/codegen path and records unroll metadata when hyperplanes are marked. Supporting either flag as a positive result would therefore require a checked `polopt` codegen feature, not just oracle pass-through.
...
| `tile.sizes` | Supported as Pluto's legacy implicit file | Pluto reads this file from the working directory and uses its positive integers as first-level tile sizes. With `--second-level-tile`, later integers are second:first tile-size ratios. PolOpt accepts this file on checked tiled routes because the tiling validator rechecks the produced tile-size witness. | Already supported for ordinary checked tiling. | Prefer a future explicit `--tile-sizes FILE` interface for hermetic artifact runs. |
...
2. Add remaining route-level surface gaps.
   - bare `--identity` policy remains explicit: require `--identity --notile`
     or `--identity --tile`
   - mixed `--identity --tile` compositions with parallel, diamond, or
     second-level tiling
...
```text
[pluto-compat-suite] OK (70 checks)
```
...
The current `polopt` surface already covers the core checked subset: affine scheduling, ordinary tiling, Pluto `tile.sizes` tile-size control, Pluto `.fst` fusion-structure control, Pluto `.precut` partial-transformation control, identity tiling with checked ISS composition, second-level tiling, ISS, one-loop parallelization, `--multipar` parallelization up to two certified dimensions, sequential diamond tiling, full-diamond mode, conditional Candl dependence testing, conservative `--candldep --scalpriv` pass-through, and LP/DFP-family pass-through on the pinned GLPK-enabled Pluto baseline.
