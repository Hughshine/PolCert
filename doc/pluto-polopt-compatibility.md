...
| `--multipar` | Supported on checked `--parallel` routes, up to two certified current dimensions | The driver passes the flag to Pluto, parses OpenScop parallel loop hints, builds a candidate dimension list, and calls the extracted verified `RawParallelCurrentMany*` compiler config. The Coq route filters certifiable dimensions before checked multi-cert parallel codegen. The cap of two matches Pluto's current OpenMP pragma extraction. | Supported for ordinary, second-level, identity-tiled, diamond, and ISS-aware parallel routes through `compile_verified_correct` plus the `Opt_parallel_current_many*` route theorems. | Broaden fixtures and add strict-mode checks. |
...
| `--diamond-tile --multipar` | Supported narrow | The diamond route feeds candidate current dimensions through the verified `RawParallelCurrentManyDiamond*` wrapper path into checked multi-cert parallel codegen. | Current fixtures pass; proof surface includes `Opt_parallel_current_many_diamond_correct` and the ISS variant. | Broaden beyond the current batch fixtures. |
