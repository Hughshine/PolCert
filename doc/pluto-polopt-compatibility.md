...
- `--ufactor` probe:
  ...
  - without `--determine-tile-size`, the same flag is accepted with checked `--unrolljam`; constant-bound loops are fully unrolled and variable-bound sequential loops use the verified block/remainder unroll route
- `--unrolljam` probe:
  ...
  - exact inner-loop jam/fusion remains a separate semantic extension: it reorders nested-loop iterations and needs a dependence/commutation certificate, unlike the current block route which preserves the original nested-loop order
...
| `--unrolljam` | Supported narrow | Pluto performs loop-body rewriting after codegen. PolOpt implements checked Loop IR post passes: complete unrolling for constant bounds, verified block/remainder unroll for variable-bound sequential loops, and verified cleanup. | Yes for the checked block/remainder Loop IR route; exact inner-loop jam/fusion still needs a dependence certificate. | Add dependence-checked inner-loop jam/fusion only with an explicit legality proof; add first-class stride-loop IR only if exact Pluto-style stride syntax becomes proof-facing. |
...
| `--ufactor` | Supported with `--determine-tile-size` on tiled routes; supported as the checked `--unrolljam` block factor otherwise | Pluto's automatic tile-size model rounds candidate sizes to a multiple of `ufactor`, even when `--nounrolljam` disables the post-codegen unroll-jam transform. Native compatibility mode passes `--ufactor` to Pluto only in this tile-size-model context. With `--unrolljam` and no `--determine-tile-size`, it is not passed to the scheduler oracle and is used by the verified LoopUnroll post pass. | Already supported for checked tile-size routes and checked unroll routes. | Add inner-loop jam/fusion only with a dependence certificate. |
...
3. Treat semantic extensions as separate projects.
   - dependence-checked inner-loop jam/fusion beyond the current
     order-preserving block/remainder unroll
