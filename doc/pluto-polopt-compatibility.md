...
| `--unrolljam` | Supported narrow | Pluto performs loop-body rewriting after codegen. PolOpt implements checked Loop IR post passes: complete unrolling for constant bounds, block/remainder unroll for variable-bound sequential loops, same-bound sibling-loop jam fusion through a per-candidate extracted validator, and verified cleanup. | Yes for the checked block/remainder route and for true jam shape when the local candidate validates; otherwise PolOpt falls back only for that candidate. | Broaden direct-Pluto effect fixtures; add first-class stride-loop IR only if exact Pluto-style stride syntax becomes proof-facing. |
...
3. Treat semantic extensions as separate projects.
   - broader direct-Pluto unroll-jam effect fixtures beyond the current
...
specific: the unroll-jam route still needs a broader direct-Pluto effect corpus,
...
schedules that depend on privatized scalar instances. Frontend and backend flags
should remain outside the optimizer compatibility surface.
