...
| `--second-level-tile` | Supported across the checked tiling matrix | The unified dispatcher covers ordinary/ISS, identity-tiled, mixed-depth, parallel/current, vector, multipar, and diamond/full-diamond producers and consumers. Exact grouped and interleaved second-level target layouts are checked statementwise before whole-program permutability validation; all known successful matrix cases are direct-band positive. | Covered by the 58-case second-level manifest, the 20-case diamond/current/strict matrix, and focused exact-rejection checks. | Keep exact route and effect assertions as fixtures broaden. |
