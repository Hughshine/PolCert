...
- Executed native compatibility mode:
  ...
  - default GLPK-enabled Pluto baseline result: `133 / 133` checks passed
...
- Executed diamond validation suite:
  ...
  - result: 3 diamond-effect cases were accepted, 3 tiling candidates were
    rejected exactly by the checked dispatcher, 2 no-effect cases were
    accepted, and 11 unsupported Pluto inputs were rejected by the frontend
- Identity composition exploration:
  ...
  - `--identity --tile --second-level-tile` and its ISS form enter the common
    band-first dispatcher and are accepted by the permutability layer while
    preserving the 256/32 second-level tile shape
  - over 69 regression `.loop` fixtures, direct Pluto `--identity --tile --diamond-tile` produced the same C output as `--identity --tile`; no identity-diamond-specific output effect was found
  - over 69 regression `.loop` fixtures, `--identity --tile` and `--identity --tile --iss` both succeeded with identical optimized loops; no ISS-only or ISS-different identity-tiling fixture was found in that bounded corpus
...
| `--identity --tile --second-level-tile` | Runs identity extraction and Pluto's tile-only second-level oracle, then enters the common band-first tiling dispatcher. The source-like second-level checker validates the complete composed schedule and produces the expected 256/32 outer-first shape. | native compat `identity-second-level`; second-level route suite |
| `--identity --tile --iss --second-level-tile` | Runs checked ISS complete-cut recovery before the same band-first second-level identity-tiling route. The final adopted route is reported explicitly. | native compat `identity-second-level-iss`; second-level route suite |
| `--iss` | Runs ISS + affine + tiling route when the input satisfies ISS shape constraints. | existing ISS suite |
| `--second-level-tile` | Selects the hierarchical producer and enters the same permutable-band-first dispatcher used by ordinary tiling. It composes with ordinary/ISS, identity-tiled, diamond/full-diamond, parallel, vector, and multipar routes covered by the extracted compiler configuration. | native compat `second-level`; second-level suite |
...
| `--prevector`, `--vector` | Consumes Pluto's innermost vector-loop hint, maps it to the generated loop nest, and emits `vector for` only after the parallel/doall check and a structural innermost check. It does not search other loop dimensions; a missing or rejected hint leaves the verified sequential producer unchanged. | native compat `prevector` and `default-prevector`; `checked_vector_annotated_codegen_correct_general`; vector-current suite |
| `--vector-current d` | Runs explicit-dimension checked vector annotation and rejects a non-innermost result. This is a native PolOpt theorem-facing route, not a Pluto flag. | vector-current suite |
...
| `--diamond-tile` | Selects the phase-aligned diamond producer; its result can feed the checked sequential, parallel, vector, and multipar consumers, with or without ISS and second-level tiling on the covered routes. | native compat `diamond`; diamond suite; second-level route matrix |
...
The bare Pluto flag `--identity` needs special care. In the current Pluto source, tiling is enabled by default, so `--identity` can still reach the tiling phase. The compatible `polopt` no-tiling identity route is therefore modeled as `--identity --notile`. Plain sequential `--identity --tile` exports the identity schedule, runs Pluto's tile-only phase, and validates the tiling boundary before code generation. All identity-tiled forms, including second-level and ISS combinations, now enter the common band-first dispatcher; the second-level forms are accepted by its permutability layer. The `--identity --tile --parallel` route composes the checked tiling boundary with the extracted checked parallel validator/codegen path; `--multipar` uses the same verified compiler wrapper via a list-of-dimensions config and can certify up to two dimensions when the validated current-space program supports them.
...
| Pluto flag | Current state | Reason | Can support? | How to support |
|---|---|---|---|---|
| `--prevector` | Supported narrow | Pluto records innermost vector loops as OpenScop loop directives. PolOpt uses the directive's statement scope to map its iterator unambiguously into the generated loop nest, then certifies the resulting current depth globally with the parallel/doall checker. It emits `vector for` only when every resulting annotation is structurally innermost. A missing or rejected hint leaves the verified sequential producer unchanged. | Already supported by the checked innermost vector annotation route. | Add statement-selective annotations or specialized vector semantics only if future work needs them, outer-loop SIMD, reductions, fixed-width lanes, or backend-specific pragmas. |
...
| `--second-level-tile` | Supported across the checked tiling matrix | The unified dispatcher covers ordinary/ISS, identity-tiled, mixed-depth, parallel/current, vector, multipar, and diamond/full-diamond producers and consumers. Grouped and interleaved second-level target layouts are checked statementwise, modulo trailing schedule coordinates that are identically zero, before whole-program permutability validation; all known successful matrix cases are direct-band positive. | Covered by the 58-case second-level manifest (53 direct acceptances and five negative checks), the 20-case diamond/current/strict matrix, and focused exact-rejection checks. | Keep exact route and effect assertions as fixtures broaden. |
...
| `--identity --tile --iss` | Supported narrow | A Coq route composes checked ISS complete-cut validation with checked identity tiling over the split program. | Already supported; current positive fixture shows tiling under `--iss`. A bounded artifact search over 69 regression fixtures found no ISS-only or ISS-different identity-tiling case. | Revisit only if a new Pluto/input corpus exposes a distinct ISS-sensitive identity-tiling effect. |
| `--identity --tile --second-level-tile` | Supported narrow | A Coq route exports the identity schedule, invokes Pluto's tile-only second-level oracle, and sends the result through the common band-first tiling dispatcher before code generation. The source-like second-level mode recognizes exact zero-row differences between recipe roots and the source schedule, then checks whole-program permutability. | The ISS variant uses the same verified dispatcher and direct route. | Broaden fixtures and keep exact route assertions. |
| `--identity --tile --diamond-tile` | Unsupported no-effect composition | Direct Pluto still emits phase dumps accepted by the four-phase validator on the focused `wavefront` probe, but corpus search over 69 regression `.loop` fixtures found the `--identity --tile --diamond-tile` C output identical to `--identity --tile`: without Pluto's affine/skew scheduling phase, diamond adds no route-specific output effect. | Correct rejection for now. | Only reconsider if an identity-diamond fixture shows a distinct diamond effect and maps to a theorem-facing route. |
...
Tile size control is likely easy from a proof perspective if the validator already proves tiling for arbitrary positive sizes. Partial tiling still needs broader effect fixtures, and mixed identity+tiling compositions need route-specific theorem coverage. The current identity-composition evidence is reproducible with `python3 tools/artifact/explore_identity_compositions.py --output-root /tmp/polcert-identity-compositions`: identity+second-level is now a supported narrow route with visible 256/32 output on `fusion7`, while identity+diamond remains rejected because Pluto's identity diamond output is not distinct from ordinary identity tiling across the 69-fixture regression corpus.
...
2. Keep the route-level edge cases explicit.
   ...
   - `--identity --tile --diamond-tile` needs a distinct Pluto effect fixture
     ...
     69 regression fixtures
   - an ISS-sensitive `--identity --tile --iss` fixture is not currently known;
     the artifact exploration checked 69 regression fixtures and found no
...
```text
[pluto-compat-suite] OK (133 checks)
```
