...
- `--ufactor` probe:
  ...
  - without `--determine-tile-size`, the same flag is accepted only with the checked constant-bound `--unrolljam` subset; it is still rejected when the requested route would need Pluto's general variable-bound unroll-jam factor
- `--unrolljam` probe:
  ...
  - native `polopt --pluto-compat --unrolljam` now accepts the checked constant-bound subset through `LoopUnroll.const_unroll_correct`
  - the same flag still rejects variable-bound cases such as `matmul.loop`, because general Pluto unroll-jam needs factor-based repeated-body and remainder generation
...
- Executed native compatibility mode:
  ...
  - default GLPK-enabled Pluto baseline result: `97 / 97` checks passed
...
| `--unrolljam` on statically constant-bounded loops | Runs a checked post pass that fully unrolls constant-bound loops in the final sequential Loop IR. `--ufactor <n>` is accepted in this subset as a compatibility flag, but the theorem-facing transform is complete constant-bound unrolling rather than factor-based remainder generation. General variable-bound Pluto unroll-jam still rejects with a precise reason. | native compat `const-unrolljam-constant-loop`, `const-unrolljam-ufactor-constant-loop`, and `reject-unrolljam-variable-loop`; end-to-end C `const_unroll`; `LoopUnroll.const_unroll_correct` |
...
| `--unrolljam` | Supported narrow | Pluto performs loop-body rewriting after codegen. PolOpt now implements the checked constant-bound subset as a Loop IR post pass; it fully unrolls loops whose final sequential bounds are constants. | Partly. | Extend `LoopUnroll` from complete constant-bound unrolling to factor-based variable-bound unroll-jam with remainder generation. |
...
| `--ufactor` without `--determine-tile-size` | Supported only with checked constant-bound `--unrolljam` | With automatic tile-size selection absent, Pluto's general meaning is the unroll-jam factor. PolOpt accepts it only when `--unrolljam` maps to `LoopUnroll.const_unroll_correct`; the factor is recorded in the compatibility surface but not passed to Pluto's scheduler oracle. | Partly. | Keep rejecting variable-bound cases until factor-based checked unroll-jam exists. |
...
These are mostly not "missing Pluto optimization". They are backend features. A direct Pluto probe on `matmul.loop` shows that `--prevector` adds an OpenScop `<loop>` extension with directive `4`; PolOpt now imports that directive as a checked vector annotation. The unroll-jam probe is different: Pluto's after-scheduling OpenScop is byte-identical with and without `--unrolljam`, but the generated C differs by unrolling/jamming two loops and adding remainder loops. The current PolOpt support is therefore deliberately not pass-through: `LoopUnroll` proves a real post pass for statically constant-bounded loops, while variable-bound factor unroll-jam remains rejected. `--ufactor` is split accordingly: it is supported as an automatic tile-size-model parameter when `--determine-tile-size` is present, accepted as a compatibility flag for the constant-bound checked unroll subset, and rejected when it would require general factor/remainder unroll-jam.

The vector path deliberately follows the checked parallel route: recover or choose a vectorizable loop, validate the doall precondition, and emit a checked vector annotation. This is sound for Pluto-style prevectorization because Pluto's vector marker is derived from parallel loop analysis. The checked unroll path is separate: the current theorem handles complete unrolling of constant-bound loops; the remaining work is factor handling and remainder generation for variable bounds. The disabling flags `--noprevector` and `--nounrolljam` remain accepted because they are how callers turn off Pluto's default codegen-side behavior for checked routes.
...
| `--ufactor` | Supported with `--determine-tile-size` on tiled routes; supported as a checked constant-bound `--unrolljam` compatibility flag | Pluto's automatic tile-size model rounds candidate sizes to a multiple of `ufactor`, even when `--nounrolljam` disables the post-codegen unroll-jam transform. Native compatibility mode passes `--ufactor` to Pluto only in this tile-size-model context. With `--unrolljam` and no `--determine-tile-size`, it is not passed to the scheduler oracle and is accepted only if the verified constant-bound post pass applies. | Already supported for checked tile-size routes and the constant-bound unroll route. | Add a real factor/remainder theorem before accepting variable-bound unroll-jam cases. |
...
| `--unroll` | Pluto accepts it only as a getopt abbreviation for `--unrolljam`. | Reject and require explicit `--unrolljam`, so the checked constant-bound subset is not selected accidentally. |
...
3. Treat semantic extensions as separate projects.
   - general variable-bound unroll-jam
...
```text
[pluto-compat-suite] OK (99 checks)
```
...
The current `polopt` surface already covers the core checked subset: affine scheduling, ordinary tiling, Pluto `tile.sizes` tile-size control via implicit and explicit `--tile-sizes-file` input, automatic tile-size-model controls including `--cache-size`, `--data-element-size`, and `--ufactor` under `--determine-tile-size`, Pluto `.fst` fusion-structure control via implicit and explicit `--fusion-structure` input, Pluto `.precut` partial-transformation control via implicit and explicit `--precut-file` input, identity tiling with checked ISS, second-level, ISS+second-level, and Pluto-hinted parallel/multipar composition, second-level tiling, ISS, one-loop parallelization, checked vector annotation for Pluto `--prevector`, checked constant-bound unrolling for Pluto `--unrolljam` including `--ufactor` as a compatibility flag on that subset, `--multipar` parallelization up to two certified dimensions, sequential diamond tiling, full-diamond mode, conditional Candl dependence testing, conservative `--candldep --scalpriv` pass-through, and LP/DFP-family pass-through on the pinned GLPK-enabled Pluto baseline.

The remaining unsatisfied optimizer-facing semantic gaps are now narrow and
specific: general variable-bound `--unrolljam` requires a factor-based PolOpt
loop/codegen post pass, and full scalar privatization requires a checked
scalar-private storage rewrite before accepting schedules that depend on
