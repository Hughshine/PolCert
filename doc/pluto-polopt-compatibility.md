...
- Identity composition exploration:
  ...
  - `--identity --tile --second-level-tile` and its ISS form enter the common
    band-first dispatcher; the current fixtures include both a band-positive
    ISS case and explicitly reported proved fallbacks, while preserving the
    256/32 second-level tile shape
...
| `--second-level-tile` | Supported across the checked tiling matrix | The unified dispatcher covers ordinary/ISS, identity-tiled, parallel/current, vector, multipar, and diamond/full-diamond producers and consumers. Uniform ordinary cases are band-positive; the matrix also includes a band-positive identity ISS case and explicit proved fallbacks for other identity-tiled, mixed-depth, and second-level diamond shapes. | Covered by the 58-case second-level manifest, the 20-case diamond/current/strict matrix, and focused exact-rejection checks. | Keep exact route and effect assertions as fixtures broaden. |
...
| `--identity --tile --second-level-tile` | Supported narrow | A Coq route exports the identity schedule, invokes Pluto's tile-only second-level oracle, and sends the result through the common band-first tiling dispatcher before code generation. Current fixtures include both band acceptance and explicitly reported proved fallback. | The ISS variant uses the same verified dispatcher. | Broaden fixtures and keep exact route assertions. |
