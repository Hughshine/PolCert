...
| `--multipar` | Supported on checked `--parallel` routes, up to two certified current dimensions | The driver passes the flag to Pluto, parses OpenScop parallel loop hints, builds a route-specific candidate list from Pluto hints and current-space scattering depth, and calls the extracted verified `RawParallelCurrentMany*` compiler config. The Coq route filters certifiable dimensions before checked multi-cert parallel codegen, so this means "up to two accepted dimensions" rather than "all requested dimensions". The cap of two matches Pluto's current OpenMP pragma extraction. | Supported for ordinary, second-level, identity-tiled, diamond, and ISS-aware parallel routes through `compile_verified_correct` plus the `Opt_parallel_current_many*` route theorems. | Broaden fixtures; strict-mode positive behavior is covered by hint-only multipar checks, and an all-hints-rejected strict fixture would further tighten the regression suite. |
...
```text
[pluto-compat-suite] OK (113 checks)
```
