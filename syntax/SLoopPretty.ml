...
let rec string_of_loop_expr env = function
  | Loop.Constant z -> string_of_z z
  | Loop.Var n -> nth_or env (Camlcoq.Nat.to_int n) ("v" ^ name_of_nat n)
  | Loop.Sum (a, b) -> Printf.sprintf "(%s + %s)" (string_of_loop_expr env a) (string_of_loop_expr env b)
  | Loop.Mult (k, e) -> Printf.sprintf "(%s * %s)" (string_of_z k) (string_of_loop_expr env e)
  | Loop.Div (e, k) -> Printf.sprintf "(%s / %s)" (string_of_loop_expr env e) (string_of_z k)
  | Loop.Mod (e, k) -> Printf.sprintf "(%s %% %s)" (string_of_loop_expr env e) (string_of_z k)
  | Loop.Max (a, b) -> Printf.sprintf "max(%s, %s)" (string_of_loop_expr env a) (string_of_loop_expr env b)
  | Loop.Min (a, b) -> Printf.sprintf "min(%s, %s)" (string_of_loop_expr env a) (string_of_loop_expr env b)

let slot_expr slots n = nth_or slots (Camlcoq.Nat.to_int n) (Loop.Constant (Camlcoq.Z.of_sint 0))

let rec string_of_affine env slots = function
  | Instr.AeConst z -> string_of_z z
  | Instr.AeVar n -> string_of_loop_expr env (slot_expr slots n)
  | Instr.AeAdd (a, b) -> Printf.sprintf "(%s + %s)" (string_of_affine env slots a) (string_of_affine env slots b)
  | Instr.AeSub (a, b) -> Printf.sprintf "(%s - %s)" (string_of_affine env slots a) (string_of_affine env slots b)
  | Instr.AeMul (k, e) -> Printf.sprintf "(%s * %s)" (string_of_z k) (string_of_affine env slots e)
...
let rec string_of_instr_expr env slots = function
...
let rec string_of_test env = function
  ...
  | Loop.And (a, b) -> Printf.sprintf "(%s && %s)" (string_of_test env a) (string_of_test env b)
  | Loop.Or (a, b) -> Printf.sprintf "(%s || %s)" (string_of_test env a) (string_of_test env b)
  | Loop.Not t -> Printf.sprintf "!(%s)" (string_of_test env t)
