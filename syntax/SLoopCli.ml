  ...
  let rec go i =
    if i >= Array.length Sys.argv then cfg
      ...
      | "--unrolljam" ->
          ...
          cfg.force_const_unroll <- true;
          add_pluto_note cfg "--unrolljam selects polopt's checked unroll post pass: constant-bound loops are fully unrolled, otherwise a verified block/remainder unroll is used on sequential Loop IR";
          go (i + 1)
