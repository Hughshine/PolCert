...
Definition check_pprog_phase_separated_ordinary_direct
  ...
  if Core.TilingCheck.check_pprog_tiling_sourceb before after ws then
    if Core.check_pprog_tiling_schedule_stripminedb before after ws then
      ...
      | Some bands =>
          if check_unique_phase_constantsb before_pis bands then
...
Proof.
  ...
  destruct (Core.infer_pinstr_list_tiling_bands before_pis ws)
    as [bands|] eqn:Hinfer.
  2:{ apply mayReturn_pure in Hcheck. discriminate. }
  destruct (check_unique_phase_constantsb before_pis bands)
    eqn:Hphases.
  2:{ apply mayReturn_pure in Hcheck. discriminate. }
  exists bands.
...
Proof.
  ...
  destruct
    ...
    as [bands [Hsource [Hshape [Hinfer [Hphases Hcomponent_check]]]]].
  destruct
  ...
  pose proof
    (check_unique_phase_constantsb_sound
       before_pis bands Hphases) as Hunique.
  pose proof
