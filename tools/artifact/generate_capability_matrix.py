...
SUPPORTED = [
    ...
    Capability("--unrolljam", "supported-narrow", "mapped to checked PolOpt post passes: constant-bound loops are fully unrolled, while variable-bound sequential loops use a verified block/remainder unroll over ordinary Loop IR followed by verified cleanup", "pluto-compat const-unrolljam-constant-loop; const-unrolljam-ufactor-constant-loop; block-unrolljam-ufactor-variable-loop; mixed-const-and-block-unrolljam; end-to-end-c const_unroll; end-to-end-c unrolljam_block_variable; LoopUnroll.const_unroll_correct; LoopUnroll.block_unroll_correct; LoopSingletonCleanup.cleanup_correct", "add dependence-checked inner-loop jam only with an explicit legality certificate; add first-class stride-loop IR only if exact Pluto-style stride syntax becomes proof-facing"),
    ...
    return {
        ...
        "summary": {
            ...
            "remaining_semantic_gaps": [
                {
                    "request": "dependence-checked inner-loop jam/fusion",
                    "reason": "The current checked block route preserves the original nested-loop execution order by repeating offset bodies and residual loops. Pluto's exact unroll-and-jam C shape can fuse inner loops, which reorders iterations and is not semantics-preserving for arbitrary Loop programs without a dependence/commutation certificate.",
                    "required_work": "Reuse or extend the doall/dependence validator to certify the relevant loop band is permutable, then prove a loop-interchange/fusion theorem that justifies the jammed order before exposing it through --unrolljam.",
                },
                {
