...
SUPPORTED = [
    ...
    Capability("--unrolljam", "supported-narrow", "mapped to checked PolOpt post passes: constant-bound loops are fully unrolled, while variable-bound sequential loops use block/remainder unroll and jam-fuse same-bound sibling loops only when the extracted jam validator accepts the original current band", "pluto-compat const-unrolljam-constant-loop; const-unrolljam-ufactor-constant-loop; block-unrolljam-ufactor-variable-loop; mixed-const-and-block-unrolljam; end-to-end-c const_unroll; end-to-end-c unrolljam_block_variable; end-to-end-c unrolljam_dependent_guard; LoopUnroll.const_unroll_correct; LoopUnroll.block_unroll_correct; LoopJamNative.jammed_two_loop_instance_refines_unjammed; LoopJamLower.try_jam_pair_exact_sound; LoopSingletonCleanup.cleanup_correct", "generalize the jam legality bridge from current-band guard to per-candidate sibling-loop certificates; add first-class stride-loop IR only if exact Pluto-style stride syntax becomes proof-facing"),
    ...
    return {
        ...
        "summary": {
            ...
            "remaining_semantic_gaps": [
                {
                    "request": "per-candidate sibling-loop jam certificates",
                    "reason": "The current route emits true jammed inner-loop shape when a conservative current-band validator accepts, and falls back to order-preserving block/remainder unroll otherwise. This is intentionally narrower than a local certificate for every same-bound sibling-loop fusion opportunity.",
                    "required_work": "Connect the sibling-loop trace theorem directly to a local affine/dependence certificate for each fused pair, instead of using the stronger current-band guard.",
                },
                {
