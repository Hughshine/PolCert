...
SUPPORTED = [
    ...
    Capability(".loop positive-literal stride range(lb, ub, step)", "supported", "front-end syntax accepts positive integer literal strides and calls extracted LoopStride lowering; the lowering keeps the extractor-facing Loop affine by using a unit-stride candidate loop plus affine guard, and checked codegen can recover the compact stride-indexed shape", "pluto-compat stride-loop-positive-literal; reject-stride-zero; reject-stride-symbolic; end-to-end-c stride_even; LoopStride.stride_loop_stmt_semantics", "extend beyond literal positive strides only after adding a way to prove/enforce step positivity"),
            ...
            "remaining_semantic_gaps": [
                {
                    ...
                    "reason": "The supported front-end stride form requires a positive integer literal. This keeps the lowered candidate domain affine and avoids adding a runtime proof obligation that the step is positive.",
                    "required_work": "Add a positivity certificate or checked precondition for affine/dynamic step expressions, then reuse the LoopStride affine-guard lowering under that certificate.",
                },
                {
