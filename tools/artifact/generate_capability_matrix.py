...
SUPPORTED = [
    ...
    Capability("--identity --tile --second-level-tile", "supported-narrow", "checked generic identity tiling route validates Pluto's second-level tile-only output and regenerates the canonical outer-first 256/32 loop shape", "pluto-compat identity-second-level and identity-second-level-matmul-init; identity-composition fusion7 support/effect probe; Identity_tiling_generic_opt_prepared_correct", "broaden further beyond directly tileable identity bands"),
    ...
    Capability("--identity --tile --parallel --multipar", "supported-narrow", "same checked identity tiling boundary, then extracted multi-cert parallel codegen for up to two distinct accepted current dimensions from Pluto-hinted/current-depth candidates", "pluto-compat identity-tiled-multipar; Opt_parallel_current_many_identity_tiled_correct; checked_annotated_codegen_many", "broaden multipar fixtures; strict-mode hint-only positive checks are covered"),
