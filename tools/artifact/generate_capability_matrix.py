...
SUPPORTED = [
    ...
    Capability("--identity --tile --second-level-tile", "supported-narrow", "the common band-first dispatcher validates Pluto's second-level tile-only output; current fixtures include both band acceptance and explicitly reported proved fallback while regenerating the canonical outer-first 256/32 loop shape", "pluto-compat identity-second-level; second-level route matrix; Opt_identity_tiled_band_correct; extracted_sequential_compile_correct", "broaden fixtures and retain exact final-route assertions"),
    ...
    Capability("--diamond-tile --second-level-tile", "supported-narrow", "second-level Pluto diamond output is checked by the same band-first diamond route with second-level witness extraction enabled; the current matrix adopts the explicit proved fallback", "pluto-compat diamond-second-level; second-level exact-route matrix; Opt_diamond_band_correct", "add a direct band-positive second-level diamond fixture if Pluto exposes one"),
    Capability("--diamond-tile --iss --second-level-tile", "supported-narrow", "ISS bridge validation composes with second-level diamond witness extraction; the current matrix adopts the explicit proved fallback", "pluto-compat diamond-iss-second-level; second-level exact-route matrix; Opt_diamond_band_with_iss_correct", "add a direct band-positive second-level diamond fixture if Pluto exposes one"),
