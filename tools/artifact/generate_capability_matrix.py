...
SUPPORTED = [
    ...
    Capability("--unrolljam", "supported-narrow", "mapped to checked PolOpt post passes: constant-bound loops are fully unrolled, while variable-bound sequential loops use a verified block/remainder unroll over ordinary Loop IR", "pluto-compat const-unrolljam-constant-loop; const-unrolljam-ufactor-constant-loop; block-unrolljam-ufactor-variable-loop; mixed-const-and-block-unrolljam; end-to-end-c const_unroll; LoopUnroll.const_unroll_correct; LoopUnroll.block_unroll_correct", "broaden effect fixtures and add first-class stride-loop IR only if exact Pluto-style stride syntax becomes proof-facing"),
