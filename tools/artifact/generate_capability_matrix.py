...
SUPPORTED = [
    Capability("default tiled route", "supported", "ordinary tiling producer through the extracted unified band-first dispatcher", "strict suite; pluto-compat ordinary-tiled; PolOptBandTiling Opt_band_correct", "keep as baseline"),
    ...
    Capability("--identity --tile --second-level-tile", "supported-narrow", "the common band-first dispatcher validates Pluto's second-level tile-only output through the direct permutability route while regenerating the canonical outer-first 256/32 loop shape", "pluto-compat identity-second-level; second-level route matrix; Opt_identity_tiled_band_correct; extracted_sequential_compile_correct", "broaden fixtures and retain exact final-route assertions"),
    Capability("--identity --tile --iss --second-level-tile", "supported", "checked ISS complete-cut validation composes with the same band-first second-level identity-tiling route", "second-level route matrix; extracted concrete pipeline bridge", "broaden performance fixtures beyond the correctness matrix"),
    Capability("--identity --tile --parallel", "supported-narrow", "checked identity tiling boundary composed with extracted checked one-dimension parallel codegen over the validated current-space program", "pluto-compat identity-tiled-parallel; checked_tiling_schedule_sourceb_first_runtime_validate_route_correct; Opt_parallel_current_identity_tiled_result_correct", "broaden fixtures"),
    Capability("--identity --tile --parallel --multipar", "supported-narrow", "same checked identity tiling boundary, then extracted multi-cert parallel codegen for up to two accepted current dimensions", "pluto-compat identity-tiled-multipar; Opt_parallel_current_many_identity_tiled_correct; checked_annotated_codegen_many", "broaden multipar fixtures; strict-mode hint-only checks are covered"),
    ...
    Capability("--second-level-tile", "supported", "checked direct-band second-level tiling route, including mixed-depth, ISS, explicit-current, and Pluto-hinted parallel compositions", "second-level suite; pluto-compat second-level-iss; pluto-compat second-level-parallel", "broaden fixtures"),
    ...
    Capability("--diamond-tile --second-level-tile", "supported-narrow", "second-level Pluto diamond output passes the structural second-level shape gate and whole-program permutability check", "pluto-compat diamond-second-level; second-level exact-route matrix; Opt_diamond_band_correct", "broaden direct-band effect fixtures"),
    Capability("--diamond-tile --iss --second-level-tile", "supported-narrow", "ISS bridge validation composes with second-level diamond witness extraction and the structural whole-program permutability route", "pluto-compat diamond-iss-second-level; second-level exact-route matrix; Opt_diamond_band_with_iss_correct", "broaden direct-band effect fixtures"),
