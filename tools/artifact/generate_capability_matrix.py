...
SUPPORTED = [
    ...
    Capability("--identity --tile --second-level-tile", "supported-narrow", "checked generic identity tiling route validates Pluto's second-level tile-only output and regenerates the canonical outer-first 256/32 loop shape", "pluto-compat identity-second-level; identity-composition fusion7 support/effect probe; Identity_tiling_generic_opt_prepared_correct", "broaden fixtures beyond directly tileable identity bands"),
    Capability("--identity --tile --iss --second-level-tile", "supported-narrow", "checked ISS complete-cut validation composes with the generic second-level identity-tiling route", "pluto-compat identity-second-level-iss; identity_tiling_generic_opt_prepared_with_iss_correct", "broaden ISS fixtures beyond the focused identity-band case"),
    ...
    Capability("--unrolljam", "supported-narrow", "mapped to checked PolOpt post passes: constant-bound loops are fully unrolled, while variable-bound sequential loops use block/remainder unroll and jam-fuse same-bound sibling loops through a per-candidate local validator; an untrusted extracted depth-plan selector controls which candidates are attempted", "pluto-compat const-unrolljam-constant-loop; const-unrolljam-ufactor-constant-loop; block-unrolljam-ufactor-variable-loop; mixed-const-and-block-unrolljam; unrolljam-empty-selector-policy; end-to-end-c const_unroll; end-to-end-c unrolljam_block_variable; end-to-end-c unrolljam_dependent_guard; direct Pluto unroll-jam effect corpus covers 6/6 native codegen effects in the default 11-fixture set and records 5 additional checked PolOpt effects where native Pluto has no clean codegen effect; LoopUnroll.const_unroll_correct; LoopUnroll.block_unroll_correct; LoopJamNative.jammed_two_loop_instance_refines_unjammed; LoopJamLower.try_jam_pair_exact_sound; LoopJamValidator.checked_loop_jam_pair_at_depth_sound; LoopSingletonCleanup.cleanup_correct", "replace the current checked-all-depths selector with a Pluto-profitability-compatible selector if exact effect policy parity is required"),
    ...
    return {
        ...
        "summary": {
            ...
            "remaining_semantic_gaps": [
                {
                    "request": "Pluto-exact unroll-jam effect policy",
                    "reason": "The checked pass now takes an untrusted depth-plan selector, and the suite shows an empty selector can suppress all unroll-jam effects while the command still succeeds. The default selector remains checked-all-depths, so the direct Pluto effect corpus still records cases where PolOpt applies an additional checked block/jam post-pass while native Pluto's codegen does not visibly change.",
                    "required_work": "Replace the default selector with a Pluto-profitability-compatible selector, including temporal-reuse/register-cost eligibility, while keeping the existing checked LoopUnroll/Jam proof as the correctness gate.",
                },
                {
                    "request": "first-class Loop IR stride loops",
                    "reason": "Current unroll-jam support intentionally lowers block/remainder unrolling into ordinary unit-stride Loop.Loop plus affine index reconstruction, so downstream validators and codegen proofs do not need a new public Loop.stmt constructor.",
                    "required_work": "Add a positive-stride Loop constructor and semantics, then update substitution, cleanup, codegen, parallel/vector tag erasure, extraction, and every affected preservation theorem before exposing it to validators.",
                },
                {
