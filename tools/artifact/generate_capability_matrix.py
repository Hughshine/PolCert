...
SUPPORTED = [
    ...
    Capability("--unrolljam", "supported-narrow", "mapped to checked PolOpt post passes: constant-bound loops are fully unrolled, while variable-bound sequential loops use a verified guarded factor peel plus residual loop; this is still narrower than Pluto's full block/remainder unroll-jam", "pluto-compat const-unrolljam-constant-loop; const-unrolljam-ufactor-constant-loop; peel-unrolljam-ufactor-variable-loop; mixed-const-and-peel-unrolljam; end-to-end-c const_unroll; LoopUnroll.const_unroll_correct; LoopUnroll.peel_unroll_correct", "extend from peel-unroll to Pluto-style repeated block plus remainder generation"),
            ...
            "remaining_semantic_gaps": [
                {
                    ...
                    "reason": "The checked subset now supports constant full unroll and variable-bound guarded peel-unroll. Pluto's full unroll-jam repeatedly unrolls each block and emits remainder handling after code generation.",
