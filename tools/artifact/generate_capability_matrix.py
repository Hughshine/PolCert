...
SUPPORTED = [
    ...
    Capability("--unrolljam", "supported-narrow", "mapped to checked PolOpt post passes: constant-bound loops are fully unrolled, while variable-bound sequential loops use a verified suffix peel cleanup; this is still narrower than Pluto's full block/remainder unroll-jam main loop", "pluto-compat const-unrolljam-constant-loop; const-unrolljam-ufactor-constant-loop; peel-unrolljam-ufactor-variable-loop; mixed-const-and-peel-unrolljam; end-to-end-c const_unroll; LoopUnroll.const_unroll_correct; LoopUnroll.suffix_peel_unroll_correct", "extend from suffix cleanup peel to Pluto-style repeated block plus jammed main-loop generation"),
    ...
    return {
        ...
        "summary": {
            ...
            "remaining_semantic_gaps": [
                {
                    "request": "full Pluto-style block/remainder --unrolljam",
                    "reason": "The checked subset now supports constant full unroll and variable-bound suffix cleanup peel. Pluto's full unroll-jam additionally repeatedly unrolls and jams the main loop body before cleanup.",
                    "required_work": "Implement block-loop factor handling, repeated-body generation for every block, remainder generation, and a semantic preservation theorem.",
                },
                {
