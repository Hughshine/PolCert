...
SUPPORTED = [
    ...
    Capability("--unrolljam", "supported-narrow", "mapped to a checked PolOpt post pass that fully unrolls statically constant-bounded loops in the final sequential Loop IR; --ufactor is accepted on that subset but does not change the full-unroll proof; variable-bound Pluto unroll-jam remains rejected", "pluto-compat const-unrolljam-constant-loop; const-unrolljam-ufactor-constant-loop; end-to-end-c const_unroll; reject-unrolljam-variable-loop; LoopUnroll.const_unroll_correct", "extend toward factor-based variable-bound unroll-jam only with a new semantic preservation theorem"),
            ...
            "remaining_semantic_gaps": [
                {
                    ...
                    "reason": "The checked subset currently fully unrolls only statically constant-bounded loops; --ufactor is accepted there only as a compatibility flag. Pluto's general unroll-jam changes generated C after the OpenScop scheduling artifact.",
