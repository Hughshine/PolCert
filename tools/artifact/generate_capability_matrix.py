...
SUPPORTED = [
    ...
    Capability("--unrolljam", "supported", "the extracted sequential endpoint composes optional constant unrolling, block/remainder unrolling, actual-domain cross-body validation within each shared parameter/enclosing-iterator environment, recursive checked jam lowering, and cleanup; the untrusted selector can choose candidates but cannot bypass local validation", "pluto-compat const-unrolljam-constant-loop; const-unrolljam-ufactor-constant-loop; block-unrolljam-ufactor-variable-loop; mixed-const-and-block-unrolljam; unrolljam-empty-selector-policy; unrolljam-context-bound-escape-rejected; end-to-end-c const_unroll; end-to-end-c unrolljam_block_variable; end-to-end-c unrolljam_dependent_guard; LoopJamBridge.checked_pair_refines_sound; LoopJamContext.checked_unrolljam_loop_with_plan_refines; ExtractedPipelineCorrect.extracted_sequential_compile_with_unrolljam_correct", "extend the checked postpass to annotated parallel/vector output before accepting those combined requests"),
