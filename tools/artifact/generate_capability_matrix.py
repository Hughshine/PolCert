...
SUPPORTED = [
    ...
    Capability("--const-unroll", "supported-sequential", "independent extracted postpass that fully expands loops whose lower and upper bounds are integer constants, then applies verified cleanup; it is not the Pluto-compatible block unroll-jam path", "pluto-compat native-const-unroll-constant-range; LoopUnroll.const_unroll_correct; extracted_sequential_compile_with_postpass_correct", "keep separate from --unrolljam and retain explicit rejection with parallel/vector output"),
